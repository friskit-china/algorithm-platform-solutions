...
class Algorithms{
    public: 
        int static binary_search(vector<int> &arr, int element, int left, int right){
            while (left < right){
...
int main(){
    ...
    std::cout<<Algorithms::binary_search(*arr_tiny, 12, 0, arr_tiny->size() - 1)<<endl;
    std::cout<<Algorithms::binary_search(*arr_large, 12, 0, arr_large->size() + 1)<<endl;
    return 0;
}